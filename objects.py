...
def begin(space, rocket_mass, screenx, ground_h, ground_w):
    class bodies():
        def __init__(self, w, h, mass, posx, posy, color):
            ...
            self.shape._set_friction(0.4)
            self.shape.color = color

            space.add(self.body, self.shape)

    rocket = bodies(rocket_w*4, rocket_h*4, rocket_mass, screenx/2, ground_h + rocket_h, THECOLORS['blue'])
    leg1 = bodies(4, 42, 10000, rocket.posx - rocket.w/2, rocket.posy - 30 - 25/2, THECOLORS['red'])
    leg2 = bodies(4, 42, 10000, rocket.posx + rocket.w/2, rocket.posy - 30 - 25/2, THECOLORS['pink'])
    ...
    class joints():
        def __init__(self, leg, pin_coord, rotary_angle0, limit_angle0, limit_angle1):
            ...
            self.rotary = pymunk.constraint.DampedRotarySpring(leg.body, rocket.body, rotary_angle0, 8e10, 8e8)
    ...
    joint1 = joints(leg1, (rocket.w/4, 10), 0, -math.pi/4 * 3, 0)
    joint2 = joints(leg2, (rocket.w/4 * 3, 10), 0, 0, math.pi/4 * 3)
